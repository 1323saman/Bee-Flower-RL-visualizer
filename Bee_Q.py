...
class RLVisualizer(tk.Tk):
    def __init__(self, agent, env):
        ...
        self.cell_size = 60
        self.title("Bee and Flower RL Visualizer")
        self.geometry(f"{self.grid_size * self.cell_size}x{self.grid_size * self.cell_size + 50}")
        
        self.bee_image = ImageTk.PhotoImage(Image.open("C://Users//lenovo//Downloads//bee.jpg.jpeg").resize((self.cell_size - 10, self.cell_size - 10)))
        self.flower_image = ImageTk.PhotoImage(Image.open("C://Users//lenovo//Downloads//flower.jpg.jpeg").resize((self.cell_size - 10, self.cell_size - 10)))
        self.sad_flower_image = ImageTk.PhotoImage(Image.open("C://Users//lenovo//Downloads//sad flower.png.jpeg").resize((self.cell_size - 10, self.cell_size - 10)))
        self.sad_bee_image = ImageTk.PhotoImage(Image.open("C://Users//lenovo//Downloads//sad bee.png.jpeg").resize((self.cell_size - 10, self.cell_size - 10)))
        self.monster_image = ImageTk.PhotoImage(Image.open("C://Users//lenovo//Downloads//monster.jpg.jpeg").resize((self.cell_size - 10, self.cell_size - 10)))
        self.obstacle_image = self.monster_image 
    ...
    def render_grid(self, final_state=None):
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                ...
                if grid[i, j] == -1:
                    self.canvas.create_image(x0 + 5, y0 + 5, anchor='nw', image=self.bee_image) 
                elif grid[i, j] == 1:
                    if final_state == "bee_wins":
                        self.canvas.create_image(x0 + 5, y0 + 5, anchor='nw', image=self.bee_image)  
                    elif final_state == "flower_catched":
                        self.canvas.create_image(x0 + 5, y0 + 5, anchor='nw', image=self.sad_flower_image)  
                    else:
                        self.canvas.create_image(x0 + 5, y0 + 5, anchor='nw', image=self.flower_image) 
                elif grid[i, j] == 2:
    ...
    def update_grid(self):
        ...
        while not done:
            ...
            if done:
                if reward == -10: 
                    self.render_grid()
                    self.canvas.create_image(state[1] * self.cell_size + 5, state[0] * self.cell_size + 5, anchor='nw', image=self.sad_bee_image)
                    self.message_label.config(text="Game Over - Bee eaten by monster!")
                elif state == self.env.goal_state:
                    self.render_grid("flower_catched")
                    self.message_label.config(text="Bee is on the Flower!")
                self.update_idletasks()
                self.update()
                time.sleep(2)
                if state == self.env.goal_state:
                    self.render_grid("bee_wins")
                    self.message_label.config(text="Bee has caught the flower successfully!")
                break
        self.render_grid("bee_wins" if state == self.env.goal_state else None)
